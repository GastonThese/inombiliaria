...
RSpec.describe User, type: :model do
  ...
  it { expect(subject).to have_one(:rented_property) }

  context "user utilities" do
    let!(:owner) { create(:user_owner_with_units) }

    it 'how to get all tenants' do
      expect(User.joins(:rented_property).uniq.count).to eq(10)
    end
    ...
    it 'how to get tenant rented unit / real state' do
      unitId = User.with_role(:tenant).first.rented_property.id
